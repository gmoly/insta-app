...
const getTripById = (id, tripService) => () => (dispatch) => {
    ...
    .then((result) => { if (result.data) { return dispatch(itemsLoaded('FETCH_TRIP_SUCCESS', {...result.data, id: id}))} }) 
...
const fetchLastTrips = (count, tripService) => () => (dispatch) => {
    ...
    .then((result) => {if (result.data) { return dispatch(itemsLoaded('FETCH_LAST_TRIPS_SUCCESS', result.data))} }) 
